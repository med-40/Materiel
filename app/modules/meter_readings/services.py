...
def _equipment_status_label(equipment: Equipment) -> tuple[str, str]:
    ...
    if value in {"unavailable", "not_working", "not-working", "stopped", "out_of_service", "out-of-service"}:
        return "لا يعمل", "danger"
    return "يعمل", "success"
...
def create_bulk_readings(db: Session, rows: Iterable[dict]):
    ...
    for index, row in enumerate(clean_rows, start=1):
        ...
        if blank_value:
            value = Decimal("0")
            warnings.append(f"الصف {row_number}: لم توجد قيمة للعداد للعتاد {registration_raw} بتاريخ {reading_date:%d/%m/%Y}؛ تم اعتبار القراءة صفرًا.")
        else:
            try:
