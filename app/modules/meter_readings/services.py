...
def history_rows(db: Session, equipment_id: int, page: int = 1, page_size: int = 20):
    ...
    ordered = (
        db.query(
            ...
            func.lag(MeterReading.odometer).over(
                partition_by=MeterReading.equipment_id,
                order_by=(MeterReading.reading_date.asc(), MeterReading.id.asc()),
            ).label("previous_odometer"),
            func.lag(MeterReading.hours).over(
                partition_by=MeterReading.equipment_id,
                order_by=(MeterReading.reading_date.asc(), MeterReading.id.asc()),
...
def create_bulk_readings(db: Session, rows: Iterable[dict]):
    ...
    if not clean_rows:
        return 0, 0, ["لم يتم إرسال أي صف صالح."]

    equipment_list = (
        db.query(Equipment)
        .options(joinedload(Equipment.equipment_type))
        .filter(Equipment.registration_number.isnot(None))
        .all()
    )
    equipment_map = {
        normalize_registration(item.registration_number): item
        for item in equipment_list
        if normalize_registration(item.registration_number)
    }
