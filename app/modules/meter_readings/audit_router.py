...
def _action(value):
    return {'add': 'إضافة قراءة', 'update': 'تعديل قراءة'}.get(value, value)
...
def operations(db: Session = Depends(get_db), current_user: User = Depends(get_current_user), action: str = Query('')):
    ...
    if action in {'add', 'update'}:
        query = query.filter(MeterReadingChange.action == action)
...
body{font-family:Arial,sans-serif;margin:20px;background:#f8fafc;color:#172b4d}h2{margin-bottom:6px}.hint{color:#64748b;font-size:13px;margin:0 0 14px}.toolbar{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0}.toolbar a{padding:8px 12px;border:1px solid #dbe3ec;border-radius:7px;background:#fff;color:#173f67;text-decoration:none}.toolbar a.active{background:#1769d1;color:#fff;border-color:#1769d1}.card{background:#fff;border:1px solid #dbe3ec;border-radius:10px;overflow:auto}table{width:100%;min-width:1050px;border-collapse:collapse}th,td{padding:9px;border:1px solid #dbe3ec;text-align:center;white-space:nowrap}th{background:#173f67;color:white}.new{font-weight:700;color:#166534}.old{font-weight:700;color:#92400e}.empty{padding:30px;text-align:center;color:#64748b}.badge{background:#eef5ff;border:1px solid #c9ddfb;border-radius:999px;padding:5px 9px;font-size:12px}
    ...
    body += '<p class="hint">هذا السجل يعرض التغيير الفعلي في بيانات العداد فقط: من قام به، متى، وما كانت القيمة قبل التعديل وما أصبحت عليه. تفاصيل ملفات الاستيراد والأخطاء ليست جزءًا من هذا السجل.</p>'
    body += '<div class="toolbar"><a href="/meter-readings">← قراءات العدادات</a>'
    body += f'<a class="{"active" if not action else ""}" href="/meter-readings/operations">كل التعديلات</a>'
    body += f'<a class="{"active" if action == "add" else ""}" href="/meter-readings/operations?action=add">الإضافات</a>'
    body += f'<a class="{"active" if action == "update" else ""}" href="/meter-readings/operations?action=update">التعديلات</a></div>'
    body += f'<p><span class="badge">عدد التغييرات المعروضة: {len(changes)}</span></p>'
    body += '<div class="card"><table><thead><tr><th>التاريخ والوقت</th><th>المستخدم</th><th>نوع التغيير</th><th>رقم التسجيل</th><th>تاريخ القراءة</th><th>القيمة السابقة</th><th>القيمة الجديدة</th><th>الوحدة</th><th>المصدر</th></tr></thead><tbody>'
    for change in changes:
        equipment = getattr(change, 'equipment', None)
        registration = '—'
        if equipment:
            registration = equipment.registration_number or equipment.asset_code or '—'
        body += f'<tr><td>{change.changed_at:%d/%m/%Y %H:%M:%S}</td><td>{escape(names.get(change.actor_id, "غير محدد"))}</td><td>{escape(_action(change.action))}</td><td>{escape(registration)}</td><td>{change.reading_date:%d/%m/%Y}' if change.reading_date else '<td>—'</td>
        body += f'</td><td class="old">{_fmt(change.old_value)}</td><td class="new">{_fmt(change.new_value)}</td><td>{"كم" if change.unit == "km" else "ساعة عمل"}</td><td>{escape(change.source or "—")}</td></tr>'
    if not changes:
        body += '<tr><td colspan="9" class="empty">لا توجد تعديلات مسجلة بعد.</td></tr>'
...
def operation_details(operation_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    raise HTTPException(410, 'تفاصيل جلسات الاستيراد ليست جزءًا من سجل تعديلات قراءات العدادات.')
