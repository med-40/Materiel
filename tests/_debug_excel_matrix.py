...
def test_debug():
    ...
    try:
        ...
        print('DEBUG_BODY=',response.body.decode('utf-8'))
        from app.modules.meter_readings.router import _read_excel_rows
        b.seek(0); rows,errs,count=_read_excel_rows(b,db); print('DEBUG_ROWS=',rows); print('DEBUG_PARSE_ERRORS=',errs,'COUNT=',count)
        assert True
    finally: db.close()
