...
def add_validation_details(db: Session, operation_id, actor_id, errors=None, warnings=None):
    ...
    if errors:
        add_event(db, operation_id, 'validation_errors', actor_id, f'تم رفض {len(errors)} صف/صفوف بسبب أخطاء في البيانات.', payload={'errors': errors})
    if warnings:
        add_event(db, operation_id, 'warnings', actor_id, f'تم تسجيل {len(warnings)} تنبيهًا دون منع الإدخال.', payload={'warnings': warnings})
...
def record_changes_for_readings(db: Session, reading_ids, actor_id, operation_id=None, source='manual'):
    """Create immutable audit rows for accepted readings. New records use NULL as old value."""
    ...
    for reading in readings:
        value, unit = _value_and_unit(reading)
        db.add(MeterReadingChange(reading_id=reading.id, equipment_id=reading.equipment_id, operation_id=operation_id, actor_id=actor_id, action='add', source=source, reading_date=reading.reading_date, unit=unit, old_value=None, new_value=value, details='إضافة قراءة جديدة إلى النظام.'))
        count += 1
    db.flush()
    return count
