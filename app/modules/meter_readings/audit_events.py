...
def _write_meter_operation(session):
    ...
    try:
        with SessionLocal() as audit_db:
            ...
            op = MeterReadingOperation(kind=kind, equipment_id=equipment_ids[0] if len(equipment_ids) == 1 else None, created_by_id=actor, status='completed', total_rows=len(readings), accepted_rows=len(readings), rejected_rows=0, reading_ids=[r.id for r in readings])
