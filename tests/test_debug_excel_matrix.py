...
def test_debug_excel_matrix():
    Base.metadata.drop_all(engine); Base.metadata.create_all(engine); db=Session()
    for reg,unit in [('E-400','km'),('E-401','hours')]:
        et=EquipmentType(name=f'نوع-{reg}',measurement_unit=unit); db.add(et); db.flush(); model=EquipmentModel(name=f'طراز-{reg}',equipment_type_id=et.id); db.add(model); db.flush(); db.add(Equipment(asset_code=f'A-{reg}',registration_number=reg,equipment_type_id=et.id,equipment_model_id=model.id,operational_status='available'))
    db.commit(); actor=User(username='debug-excel',full_name='debug',hashed_password='x',role='admin'); db.add(actor); db.commit(); db.refresh(actor)
    ...
    response=meter_readings_import_excel(UploadFile(filename='debug.xlsx',file=b),None,db,actor); b.seek(0); rows,errs,count=_read_excel_rows(b,db)
